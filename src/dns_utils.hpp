...
namespace Dns{
	...
	#define checkListLength(size, minimum) if(size < minimum) throw std::length_error("Cannot parse DNS message - need at least " + to_string(minimum) + " bytes, got " + to_string(size));
}
